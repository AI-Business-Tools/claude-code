...
├── other-articles/               <-   /kb update discovers all folders
...
**Topic folders are dynamic.** The skill does not maintain a hardcoded list. Any immediate subdirectory of `knowledge-base/` (other than `aa-inbox/`, `aa-blog/`, `aa-recents/`, and `*_build/`) is treated as a topic folder. Adding a new folder requires no skill or config changes; a sync/update run will discover it automatically.
...
Updated automatically by a sync/update run. The user's folder organization choices over time teach the system what belongs where.
...
This is a lightweight check (read index, grep, optionally append one row). It does not require invoking a full sync/update. Content skills should perform this check as a final step after writing their deliverables.
...
- **Index is append-only during processing.** Entries are only removed during a sync/update run when source files are confirmed missing.
